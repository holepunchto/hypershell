...
function ClientSocket ({ keyfile, serverPublicKey, reusableSocket = false, testnet = false }) {
  ...
  const node = new DHT({ bootstrap: testnet ? [{ host: '127.0.0.1', port: 49737 }] : undefined })
  ...
  socket.on('end', () => {
    // console.log('[client-socket] socket on end')
    socket.end()
  })
  ...
  // socket.once('close', () => console.log('[client-socket] socket on close'))
